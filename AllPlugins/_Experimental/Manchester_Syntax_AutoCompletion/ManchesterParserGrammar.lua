...
function classExpressionAutoComplition(text)
	...
	, {  
		...
		parse_Self = function (message, current_pos)    
			...
			return current_pos  end,
		parse_selfL = function (message, current_pos)    
			if checkIfClassName(message, current_pos) == false then 
				add_error_message(errors, current_pos, "self", 10)
			end
			return current_pos  end,
...
function dataRangeAutoComplition(text)
	...
	whitespace			<-	(%parse_space (" " / %nl))
		...
		parse_selfL = function (message, current_pos) add_error_message(errors, current_pos, "self", 10) return current_pos end,
...
function getText()
	--print("SSSSSSSSSSSSSSSSSSSSSSS", GetCursorPosition())
...
function displayEndings(str, errors)
	...
	if lQuery("D#Event"):attr("eventName")=="MultiLineTextBoxChange" or (lQuery("D#Event"):attr("eventName")=="Change")  then
		...
		--	print(line, lengh, GetCursorPosition())
			 local text = lQuery("Event/source"):attr("text")
			...
			if ending_type == "terminal" then 
				...
				if com ~= nil then
					print (position, "position position position position position position position position")
			...
			elseif ending_type == "all_symbol" then
				...
				if position~=-1 then 
					print(position, "farthest_pos_prev")
...
function create_error_message(str, errors)
	--print(lQuery("Event"):attr("eventName"))
	-- print(str)
	--print(dumptable(errors))
	 --print(GetCursorPosition(), "GetCursorPosition")
	-- print(lQuery("Event/form"):attr("caption"))
	
	--ja ir textArea
	local EventSourceId = lQuery("Event/source"):id()
	local textArea = lQuery("D#TextArea"):filter(function(ta)
		return ta:id()==EventSourceId
	end)

	--print(lQuery("Event/source"):log(), "jhh")
	--if lQuery("Event/form"):attr("caption")=='Annotation' or lQuery("Event/edited"):is_not_empty() or lQuery("Event/inserted"):is_not_empty() or lQuery("Event/deleted"):is_not_empty() or (GetCursorPosition()~="Click" and string.find(GetCursorPosition(), ",")==nil) then
	--if str~="" then
--	if GetCursorPosition()~= "0,0" then

	if lQuery("D#Event"):attr("eventName")=="MultiLineTextBoxChange" or (lQuery("D#Event"):attr("eventName")=="Change")  then
		if (GetCursorPosition()=="0" and lQuery("D#Event"):size()==1) then 
		else
			-- local lengh = tonumber(GetCursorPosition())--uzrakstita teksta garums lidz kursoram
			-- if lengh==0 and string.len(str)>0 then lengh = string.len(str) end
			local line = nil
			local comma = string.find(GetCursorPosition(), ",")
			if comma~=nil then 
				line = string.sub(GetCursorPosition(), comma+1)
		...
		--	print(line, lengh, GetCursorPosition())
			...
			local a, lengh = getText()
			--local len = string.len(text)--visa teksta garums
			
		--	print(len, "visa teksta garums")
			local farthest_pos = -1--talaka pozicija
			local farthest_pos_prev = -1--iepriekstalaka pozicija (izmanto, ja tika rakstits nosaukuma sakums)
			local messages_to_report = {}
			...
			--print(farthest_pos, "farthest_pos farthest_pos farthest_pos")
			...
			if lengh>=farthest_pos then 
				...
				if TermMessages[1] ~= nil then
					...
					com = tda.ShowAutoCompletionOptions(TermMessages)
					  --ja tika izvelets turpinajums
					if com ~= nil then
						local beforCursor = string.sub(text, 1, lengh-er_lenght)
						local afterCursor = string.sub(text, lengh+1)
						local main = beforCursor .. TermMessages[com+1] .. afterCursor
						
						refreshForm(main, string.len(beforCursor .. TermMessages[com+1]))--atjaunojam formu
					end
					return ""
					--ja nebija sakritibu iespejamo turpinajumu tabulaa, tad ir kluda
				else
			...
			local subStringLength
			if farthest_pos_prev~=-1 then 
				--print(farthest_pos_prev, farthest_pos, "farthest_pos_prev")
				subString = string.sub(text, farthest_pos_prev)
				--print(subString, "subString")
				subStringLength = string.len(subString)
				local label = 0
				for pos, messages in pairs(uniqueMessages) do
					if string.sub(messages, 1, subStringLength)==subString then label = 1 end
				end
				if label == 1 then
					beforCursor = string.sub(text, 1, farthest_pos_prev-1)
					afterCursor = string.sub(text, lengh+1)
				else
					beforCursor = string.sub(text, 1, lengh)
					afterCursor = string.sub(text, lengh+1)
			...
			--piedavajam logu ar iespejamiem turpinajumiem
			com = tda.ShowAutoCompletionOptions(uniqueMessages)
			--ja tika izvelets turpinajums
			if com ~= nil then
				if string.sub(uniqueMessages[com+1], 1, subStringLength)~=subString then 
					beforCursor = string.sub(text, 1, lengh)
					afterCursor = string.sub(text, lengh+1)
				end

				local main = beforCursor .. uniqueMessages[com+1] .. afterCursor
				...
				refreshForm(main, string.len(beforCursor .. uniqueMessages[com+1]))--atjaunojam formu
			end
		end
	end
