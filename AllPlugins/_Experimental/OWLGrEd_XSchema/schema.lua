...
function setPrefixesPlus(dataCompartType, dataCompartment, parsingCompartment)
	...
	if dataCompartment~=nil then
		...
		if noSchema:attr("value") ~= "true" and allValuesFrom:attr("value")~="true" then result = "&" end
	end
	return result
end
...
function setPrefixesNoSchema(compartment, oldValue)
	...
	if compartment:attr("value") == "true" then 
		...
		local parentCompartment = compartment:find("/parentCompartment")
		local allValuesFrom = parentCompartment:find("/subCompartment:has(/compartType[id='allValuesFrom'])")
		if allValuesFrom:attr("value") == true then
		    name:attr("input", name:attr("value"))
		else
		    name:attr("input", "&" .. name:attr("value"))
		end
...
function setPrefixesPlusFromAllFaluesFrom(compartment, oldValue)
	...
	if compartment:attr("value") == "true" then 
		...
		local parentCompartment = compartment:find("/parentCompartment")
		local noSchema = parentCompartment:find("/subCompartment:has(/compartType[id='noSchema'])")
		if noSchema:attr("value") ~= "true" then 
		    name:attr("input", "&" .. name:attr("value"))
		end
...
function setPrefixesNoSchemaDataProperty(compartment, oldValue)
	...
	if compartment:attr("value") == "true" then 
		local parentCompartment = compartment:find("/parentCompartment")
		core.create_missing_compartment(parentCompartment, parentCompartment:find("/compartType"), parentCompartment:find("/compartType/subCompartType[id='allValuesFrom']"))
		...
		local parentCompartment = compartment:find("/parentCompartment")
		local allValuesFrom = parentCompartment:find("/subCompartment:has(/compartType[id='allValuesFrom'])")
		if allValuesFrom:attr("value") == "false" then
			core.create_missing_compartment(parentCompartment, parentCompartment:find("/compartType"), parentCompartment:find("/compartType/subCompartType[id='hiddenCompartment']"))
			local hiddenCompartment = parentCompartment:find("/subCompartment:has(/compartType[id='hiddenCompartment'])")
			hiddenCompartment:attr("value", "true")
...
function setPrefixesPlusFromAllFaluesFromDataProperty(compartment, oldValue)
	...
	if compartment:attr("value") == "true" then 
		...
		core.create_missing_compartment(parentCompartment, parentCompartment:find("/compartType"), parentCompartment:find("/compartType/subCompartType[id='noSchema']"))
		...
		if noSchema:size() == 0 or noSchema:attr("value") == "false" then
...
function deleteCompartmentsForm(text)
    ...
    caption = "Close"
    ,eventHandler = utilities.d_handler("Click", "lua_engine", "lua.OWLGrEd_XSchema.schema.closeDeleteCompartment()")
  })
  ...
  local form = lQuery.create("D#Form", {
    ...
    ,eventHandler = utilities.d_handler("Close", "lua_engine", "lua.OWLGrEd_XSchema.schema.closeDeleteCompartment()")
    ,minimumWidth = 300
    ,maximumWidth = 300
    ,prefferedWidth = 300
	,component = {
...
function exportParametersForm()
	...
	local close_button = lQuery.create("D#Button", {
    caption = "Close"
    ,eventHandler = utilities.d_handler("Click", "lua_engine", "lua.OWLGrEd_XSchema.schema.close()")
  })
  ...
  local form = lQuery.create("D#Form", {
    ...
    ,cancelButton = close_button
    ,defaultButton = close_button
    ,eventHandler = utilities.d_handler("Close", "lua_engine", "lua.OWLGrEd_XSchema.schema.close()")
    ,minimumWidth = 410
	,component = {
		lQuery.create("D#HorizontalBox",{
			id = "HorizontalAllForms"
											...
											lQuery.create("D#VerticalBox", {
												...
												,component = {
													lQuery.create("D#RadioButton", {
														...
														,eventHandler = utilities.d_handler("Click", "lua_engine", "lua.OWLGrEd_XSchema.schema.saveRadioButtonParameter()")
													})
													,lQuery.create("D#RadioButton", {
										...
										,lQuery.create("D#Row", {
											...
											,component = {
												lQuery.create("D#CheckBox", {
													...
													,eventHandler = utilities.d_handler("Change", "lua_engine", "lua.OWLGrEd_XSchema.schema.saveCheckBoxParameter()")
												})
...
function onAttributeOpen(form)
	...
	if attribute:find("/subCompartment"):size() == 0 then
		core.create_missing_compartment(attribute, attribute:find("/compartType"), attribute:find("/compartType/subCompartType[id='allValuesFrom']"))
		local allValuesFrom = attribute:find("/subCompartment:has(/compartType[id='allValuesFrom'])")
		allValuesFrom:attr("value", "true")
		core.update_compartment_input_from_value(allValuesFrom)

		allValuesFrom:link("component", form:find("/component[id='allValuesFrom']/component/component[id='field']"))
		allValuesFrom:find("/component"):attr("checked", "true")	
	end
...
function setExportAxioms()
	...
	-- if parameterValue == "Standart (non-shema) ontology only" then
